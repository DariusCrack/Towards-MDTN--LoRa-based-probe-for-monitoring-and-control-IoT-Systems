    ...
    if uart_rpi.any():
        ...
        if data_pi:
            ...
            if b"</RPI_METRICS>" in partial or b"</CMD_RESPONSE>" in partial:
                ...
                partial = b""
        time.sleep(0.1)
        payload_pi = content.encode('utf-8')
        filename = 'metrics_Rpi.json'
        file = CTP_File(
            name=filename,
            content=payload_pi,
...
#         data_pi = partial_r.decode().strip()
#         body = data_pi.split("<RPI_METRICS>")[1].split("</RPI_METRICS>")[0]
#         partial_r = b""
#         return "📡 RPI_METRICS\n" + body
#     elif b"</CMD_RESPONSE>" in partial_r:
#         data_pi = partial_r.decode().strip()
#         body = data_pi.split("<CMD_RESPONSE>")[1].split("<CMD_RESPONSE>")[0]
#         partial_r = b""
#     return None
...
def uart_fun_lily():
    ...
    # To encode and print in the SHELL
    if uart_lily.any():
        try:
...
#         parse_lily()
    ...
    # To encode and send to the Gateway
#     if data_lily:
        payload_lily = data_lily.encode('utf-8')
        filename = 'metrics_LilyGo.json'
        file = CTP_File(
        name=filename,
